...
| `startTracking(options)` | Rejects missing prerequisites and permissions; does not request permission. | Starts foreground-service tracking with title, text, and icon notification options; the command completes on `onStart`, fails on `onFail`, and times out if the native SDK never acknowledges. A timed-out attempt is invalidated and must be explicitly recovered before retry. | Starts the tracking manager with additional audio-session options; late callbacks from timed-out or superseded attempts are discarded. | Rechecks the current battery-exemption status, then returns `Future<void>` only after native start acknowledgement; `TrackingCreatedEvent` carries the session identity. |
